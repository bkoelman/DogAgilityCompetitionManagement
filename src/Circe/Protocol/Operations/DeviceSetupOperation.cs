...
{
    /// <summary>
    /// This operation is used by a controller during the manufacturing process of a new mediator or other new wireless
    /// hardware device to assign its unique network address.
    /// </summary>
    /// <remarks>
    /// A controller may send this operation in response to a Keep Alive (51) operation from a mediator. Upon receipt of this
    /// operation, the mediator assigns the included address to itself when capabilities are omitted. Otherwise, the included
    /// address is assigned to the wireless device that matches the destination address. When destination address is missing,
    /// the mediator assumes address 000000.
    /// </remarks>
    [Serializable]
    public sealed class DeviceSetupOperation : Operation
    {
        ...
        private readonly NetworkAddressParameter destinationAddressParameter =
            ParameterFactory.Create(ParameterType.NetworkAddress.DestinationAddress, false);

        [NotNull]
        private readonly NetworkAddressParameter assignAddressParameter =
            ParameterFactory.Create(ParameterType.NetworkAddress.AssignAddress, true);

        [NotNull]
        private readonly IntegerParameter capabilitiesParameter =
            ParameterFactory.Create(ParameterType.Integer.Capabilities, false);
        ...
        [NotNull]
        public WirelessNetworkAddress DestinationAddressOrDefault
            => DestinationAddress ?? WirelessNetworkAddress.Default;
        ...
        public WirelessNetworkAddress AssignAddress
        {
            ...
            set
            {
                assignAddressParameter.Value = value?.Value;
            }
        }
